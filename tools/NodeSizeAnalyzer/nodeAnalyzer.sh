...
main() {
    ...
    if [[ "$1" == "analyze" ]]; then
        node_size_analyzer
        # Резюме анализа размера узла
        echo -e "${GREEN}======================================="
        echo -e "${CYAN}Резюме анализа размера узла:${NC}"
        echo -e "Обнаружено ${#nodes[@]} узлов."
        echo -e "Общий размер всех узлов: ${total_size_nodes}GB (${percentage_used_nodes}%) от общего пространства диска."
        echo -e "${GREEN}=======================================${NC}"
    ...
    echo -e "Скрипт успешно проанализировал систему. Обязательно проверьте большие файлы на предмет возможности их удаления или архивации для освобождения места. Если вы проводили анализ узлов, то они занимают определенный процент от общего пространства диска."
    echo -e "${GREEN}=======================================${NC}"
}
